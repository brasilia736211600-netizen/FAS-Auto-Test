...
cp "$SOURCE_DIR/model_router.py" "$SOURCE_DIR/fas_runtime.py" "$SOURCE_DIR/fas_fallback.py" \
   ...
   "$SOURCE_DIR/fas_remote.py" "$SOURCE_DIR/fas_resume.py" "$SOURCE_DIR/fas_autopilot.py" "$FAS_HOME/"
...
cat > "$BIN_DIR/fas-autopilot" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
FAS_HOME="${FAS_HOME:-$HOME/.local/share/fas}"
export PYTHONPATH="$FAS_HOME${PYTHONPATH:+:$PYTHONPATH}"
exec python3 "$FAS_HOME/fas_autopilot.py" "$@"
EOF
chmod 755 "$BIN_DIR/fas-autopilot"
...
echo "Autopilot: $BIN_DIR/fas-autopilot"
